...
const { mondayOf, weekDatesOf } = await import("../src/modules/web/pages/heatmap/heatmapWeekPanel.js");

test("mondayOf de un miércoles devuelve el lunes de esa misma semana", () => {
  const wednesday = new Date(2026, 7, 5); // 5 ago 2026 — miércoles
...
test("mondayOf de un lunes devuelve el mismo día", () => {
  const monday = new Date(2026, 7, 3); // 3 ago 2026 — lunes
...
test("mondayOf de un domingo devuelve el lunes anterior (semana lunes-domingo)", () => {
  const sunday = new Date(2026, 7, 9); // 9 ago 2026 — domingo
...
test("weekDatesOf cruza el límite de mes correctamente", () => {
  // 1 sep 2026 es martes; su semana es 31 ago (lunes) a 6 sep (domingo).
